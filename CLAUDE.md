...
- ThemeToggleComponent (`ui_theme_toggle`) renders Light, Dark and System buttons wired to the `theme-toggle` Stimulus controller, which sets `data-theme` on the `html` element and stores the choice in the `keystone_theme` cookie. `KeystoneUi::ThemeChoice` reads that cookie on the server, and `keystone_theme_attributes` writes the mark onto the layout's `html` tag so pages open in the chosen mode.
